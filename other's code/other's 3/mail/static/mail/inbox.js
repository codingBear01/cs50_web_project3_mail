...
function reply_email(email) {
  ...
  document.querySelector('#compose-body').value =
    pre_body_text + email['body'].replace(/^/gm, '\t');
}
...
function load_email(email_id, origin_mailbox) {
  ...
  fetch(`/emails/${email_id}`)
    ...
    .then((email) => {
      ...
      ['subject', 'timestamp', 'sender', 'recipients', 'body'].forEach(
        (email_element) => {
          ...
          if (email_element === 'subject') {
            ...
            (origin_mailbox === 'sent'
              ? data_for_potential_buttons_to_add.slice(0, 1)
              : data_for_potential_buttons_to_add
      ...
      back_button_col_div.innerHTML = `<p>${
        origin_mailbox.charAt(0).toUpperCase() + origin_mailbox.slice(1)
      }</p>`;
      back_button_col_div.addEventListener('click', () =>
        load_mailbox(origin_mailbox)
      );
      back_button_row_div.append(back_button_col_div);
      document
        .querySelector('#single-email-back-section')
        .append(back_button_row_div);
    })
  ...
  fetch(`/emails/${mailbox}`)
    ...
    .then((emails) => {
      ...
      emails.forEach((email) => {
        ...
        if (email !== artificial_first_email) {
          row_div_element.addEventListener('click', () =>
  ...
  fetch('/emails', {
    ...
    .then((result) => {
      ...
      if ('error' in result) {
        ...
        document.querySelector('#to-text-error-message').innerHTML =
          result['error'];
      }
      console.log(result);
